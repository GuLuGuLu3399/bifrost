...
const totalHits = computed(() => {
  return searchResult.value?.total_hits ?? searchResult.value?.totalHits ?? 0;
});

const tookMs = computed(() => {
  return searchResult.value?.took_ms ?? searchResult.value?.tookMs ?? 0;
});
...
<template>
  <main class="min-h-screen bg-white text-gray-900">
    <div class="max-w-4xl mx-auto px-6 py-12">
      ...
      <!-- No results -->
      <div v-else-if="searchResult && searchResult.hits.length === 0" class="text-center py-12">
      ...
      <div v-else-if="searchResult" class="space-y-6">
        ...
        <!-- Search hits -->
        <article v-for="hit in searchResult.hits" :key="hit.id"
          class="border border-gray-200 rounded-lg p-6 hover:shadow-lg transition-shadow">
          <div class="mb-2 flex items-center gap-2">
            <span class="text-xs font-semibold text-gray-500">
              相关度: {{ (hit.score * 100).toFixed(0) }}%
            </span>
            <span v-if="hit.published_at || hit.publishedAt" class="text-xs text-gray-400">
              · {{ formatDate((hit.published_at || hit.publishedAt) as string) }}
            </span>
          </div>

          <NuxtLink :to="`/posts/${hit.slug}`" class="block group mb-3 hover:no-underline">
            <h2 v-if="hit.highlight_title" class="text-xl font-semibold text-gray-900 group-hover:text-blue-600"
              v-html="hit.highlight_title" />
            <h2 v-else class="text-xl font-semibold text-gray-900 group-hover:text-blue-600">
              {{ hit.title }}
            </h2>
          </NuxtLink>

          <!-- Highlight content -->
          <div v-if="hit.highlight_content" class="text-gray-600 line-clamp-3 mb-3" v-html="hit.highlight_content" />
        </article>

        <!-- Facets (if available) -->
        <aside v-if="searchResult.facets" class="mt-12 p-6 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <div v-if="searchResult.facets.categories" class="mb-6">
            ...
            <div class="flex flex-wrap gap-2">
              <button v-for="(count, categoryName) in searchResult.facets.categories" :key="categoryName"
          ...
          <div v-if="searchResult.facets.tags">
            ...
            <div class="flex flex-wrap gap-2">
              <button v-for="(count, tagName) in searchResult.facets.tags" :key="tagName"
                class="px-3 py-1 text-sm bg-white border border-gray-300 rounded hover:bg-blue-50 transition-colors">
                {{ tagName }} ({{ count }})
              </button>
            </div>
