...
CREATE TABLE IF NOT EXISTS raw_events (
    ...
    -- 元数据
    meta TEXT,                                  -- 元数据，存储为 JSON 字符串（避免扩展依赖）
    ip VARCHAR(45),                             -- 提取 IP 到独立列便于查询（IPv4/IPv6）
    user_agent TEXT,                            -- 提取 User-Agent 到独立列
    referrer TEXT,                              -- 引用来源
    
    -- 时间相关字段
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,  -- DuckDB 时间戳（无时区）
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    event_date DATE                             -- 存储事件日期（应用层计算）
);
...
CREATE TABLE IF NOT EXISTS daily_stats (
    ...
    -- 元数据
    first_event_at TIMESTAMP,                   -- 当天首次事件时间
    last_event_at TIMESTAMP,                    -- 当天最后事件时间
...
CREATE TABLE IF NOT EXISTS hourly_stats (
    date_hour TIMESTAMP NOT NULL,            -- 按小时聚合
...
CREATE TABLE IF NOT EXISTS user_behavior (
    ...
    -- 活跃度指标
    active_duration INTEGER,                    -- 活跃时长（秒，DuckDB 无 INTERVAL）
    first_active_at TIMESTAMP,                  -- 首次活跃时间
...
-- DuckDB 不支持 PL/pgSQL 触发器，用应用层更新 updated_at
-- 触发器逻辑已移至应用层（Rust）维护
...
-- 视图：热门内容统计（基于 raw_events 的简单查询）
-- DuckDB 中使用普通视图代替物化视图，通过应用层缓存优化
-- 应用层可定期执行此查询并缓存结果
CREATE VIEW IF NOT EXISTS hot_content_daily_view AS
SELECT 
    target_id,
    COUNT(*) as pv_count,
...
GROUP BY target_id, CAST(created_at AS DATE);